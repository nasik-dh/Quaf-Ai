...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
        ...
        // Check if command is a pic request
function isPicRequest(cmd) {
    const parts = cmd.split(' ');
    if (parts.length === 2 && parts[1].toLowerCase() === 'pic') {
        return isValidStudentId(parts[0]);
    }
    return false;
}

// Extract student ID from pic command
...
• 📎 ATTACH - Upload contact file for bulk messaging
• send_gmail - Send bulk Gmail messages
• send_whatsapp - Send bulk WhatsApp messages
...
function forceShowResults() {
    ...
    if (!userSession.firstCommand) {
        ...
        if (cmd === "quiz") {
            ...
            // Early return because we've reset the output
            return;
        } else if (cmd === "contact_format" || cmd === "format") {
            response = `
CONTACT FILE FORMAT:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
John Doe,john@email.com,+1234567890,ABC Corp,VIP
Jane Smith,jane@email.com,+919876543210,XYZ Ltd,Regular

📱 PHONE FORMAT: +CountryCodeNumber (MUST include country code)
   Examples: +919876543210, +1234567890, +447123456789
📧 EMAIL FORMAT: valid@email.com

💡 MINIMUM REQUIRED: Name + (Email OR Phone)
⚠️  WhatsApp requires international format with country code
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
        } else if (cmd === "send_gmail" || cmd === "send gmail") {
            await sendGmailCampaign();
            return;
        } else if (cmd === "send_whatsapp" || cmd === "send whatsapp") {
...
// Handle file selection for contacts
document.getElementById('fileInput').addEventListener('change', function(event) {
    const files = event.target.files;
    if (files.length > 0) {
        processContactFile(files[0]);
    }
});

// Process contact file (CSV, TXT, Excel)
async function processContactFile(file) {
    const output = document.getElementById("commandOutput");
    const timestamp = new Date().toLocaleTimeString();
    
    output.innerHTML += `\n[${timestamp}] PROCESSING CONTACT FILE...\n`;
    output.innerHTML += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    output.innerHTML += `📏 SIZE: ${fileSize} KB\n`;
    output.innerHTML += `📋 TYPE: ${fileType.toUpperCase()}\n\n`;
    ...
    try {
        ...
        if (fileType === 'csv' || fileType === 'txt') {
            ...
            contacts = parseContactText(text);
        } else if (fileType === 'xlsx' || fileType === 'xls') {
            output.innerHTML += `⚠️  Excel files need to be converted to CSV format\n`;
            output.innerHTML += `💡 Save your Excel file as CSV and try again\n\n`;
            return;
        } else {
            output.innerHTML += `❌ UNSUPPORTED FILE TYPE\n`;
            output.innerHTML += `✅ Supported: CSV, TXT\n\n`;
            return;
        }
        
        if (contacts.length > 0) {
            uploadedContacts = contacts;
            output.innerHTML += `✅ CONTACTS LOADED SUCCESSFULLY!\n`;
            output.innerHTML += `📊 TOTAL CONTACTS: ${contacts.length}\n\n`;
            
            // Show sample contacts
            output.innerHTML += `📋 SAMPLE CONTACTS:\n`;
            contacts.slice(0, 5).forEach((contact, index) => {
                output.innerHTML += `${index + 1}. ${contact.name} - ${contact.email || contact.phone}\n`;
            });
            
            if (contacts.length > 5) {
                output.innerHTML += `... and ${contacts.length - 5} more\n`;
            }
            
            output.innerHTML += `\n🚀 READY FOR BULK MESSAGING!\n`;
            output.innerHTML += `📧 Type 'send_gmail' for email campaign\n`;
            output.innerHTML += `📱 Type 'send_whatsapp' for WhatsApp campaign\n\n`;
            
        } else {
            output.innerHTML += `❌ NO VALID CONTACTS FOUND\n`;
            output.innerHTML += `💡 Check file format and try again\n\n`;
        }
        
    } catch (error) {
        output.innerHTML += `❌ ERROR PROCESSING FILE: ${error.message}\n\n`;
    }
...
// Replace the existing isValidPhone function with this:
...
// Add this new function after isValidPhone
function formatPhoneForWhatsApp(phone) {
    ...
    if (!cleanPhone.startsWith('+')) {
        // Add default country code (you can change this based on your needs)
        cleanPhone = '+91' + cleanPhone; // +91 for India, change as needed
    }
...
// Send Gmail campaign
async function sendGmailCampaign() {
    const output = document.getElementById("commandOutput");
    const timestamp = new Date().toLocaleTimeString();
    
    if (uploadedContacts.length === 0) {
        output.innerHTML += `\n[${timestamp}] ❌ NO CONTACTS LOADED\n`;
        output.innerHTML += `📎 Please upload a contact file first\n\n`;
        return;
    }
    
    output.innerHTML += `\n[${timestamp}] INITIATING GMAIL BULK CAMPAIGN...\n`;
    output.innerHTML += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    
    const emailContacts = uploadedContacts.filter(contact => contact.email && isValidEmail(contact.email));
    
    output.innerHTML += `📊 CAMPAIGN STATISTICS:\n`;
    output.innerHTML += `   Total Contacts: ${uploadedContacts.length}\n`;
    output.innerHTML += `   Valid Emails: ${emailContacts.length}\n`;
    output.innerHTML += `   Invalid/Missing: ${uploadedContacts.length - emailContacts.length}\n\n`;
    
    if (emailContacts.length === 0) {
        output.innerHTML += `❌ NO VALID EMAIL ADDRESSES FOUND\n\n`;
        return;
    }
    
    // Create mailto links for bulk sending
    const batchSize = 50; // Gmail limit
    const batches = [];
    
    for (let i = 0; i < emailContacts.length; i += batchSize) {
        batches.push(emailContacts.slice(i, i + batchSize));
    }
    
    output.innerHTML += `📧 GMAIL BULK SENDER GENERATED:\n`;
    output.innerHTML += `📦 Batches: ${batches.length} (max ${batchSize} emails per batch)\n\n`;
    
    batches.forEach((batch, index) => {
        const emails = batch.map(contact => contact.email).join(',');
        const subject = encodeURIComponent(`Message from ${userSession.userName}`);
        const body = encodeURIComponent(`Hello,\n\nThis is a message from ${userSession.userName}.\n\n[Your message here]\n\nBest regards,\n${userSession.userName}`);
        
        const mailtoLink = `mailto:${emails}?subject=${subject}&body=${body}`;
        
        output.innerHTML += `📨 BATCH ${index + 1} (${batch.length} recipients):\n`;
        output.innerHTML += `<a href="${mailtoLink}" target="_blank" style="color: #00ff00; text-decoration: underline; font-weight: bold;">🚀 SEND BATCH ${index + 1}</a>\n\n`;
    });
    
    output.innerHTML += `⚠️  IMPORTANT INSTRUCTIONS:\n`;
    output.innerHTML += `1. Click each batch link above\n`;
    output.innerHTML += `2. Your default email client will open\n`;
    output.innerHTML += `3. Edit the message content as needed\n`;
    output.innerHTML += `4. Send each batch manually\n`;
    output.innerHTML += `5. Wait between batches to avoid spam detection\n\n`;
    
    // Log campaign to admin
    const campaignLog = `
📧 GMAIL BULK CAMPAIGN INITIATED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👤 User: ${userSession.userName} (${userSession.userEmail})
📊 Stats: ${emailContacts.length} valid emails in ${batches.length} batches
⏰ Time: ${new Date().toLocaleString()}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    
    await sendMessageToSheet(campaignLog);
...
async function sendWhatsAppCampaign() {
    ...
    output.innerHTML += `\n[${timestamp}] INITIATING WHATSAPP BULK CAMPAIGN...\n`;
    output.innerHTML += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    
    const phoneContacts = uploadedContacts.filter(contact => contact.phone && isValidPhone(contact.phone));
    
    output.innerHTML += `📊 CAMPAIGN STATISTICS:\n`;
    output.innerHTML += `   Total Contacts: ${uploadedContacts.length}\n`;
    output.innerHTML += `   Valid Phones: ${phoneContacts.length}\n`;
    output.innerHTML += `   Invalid/Missing: ${uploadedContacts.length - phoneContacts.length}\n\n`;
    
    if (phoneContacts.length === 0) {
        output.innerHTML += `❌ NO VALID PHONE NUMBERS FOUND\n\n`;
        return;
    }
    
    const message = encodeURIComponent(`Hello! This is a message from ${userSession.userName}.\n\n[Your message here]\n\nBest regards!`);
    
    output.innerHTML += `📱 WHATSAPP BULK SENDER:\n\n`;
    
    // Replace the WhatsApp link generation part in sendWhatsAppCampaign function:
phoneContacts.forEach((contact, index) => {
    const whatsappPhone = formatPhoneForWhatsApp(contact.phone);
    const whatsappLink = `https://wa.me/${whatsappPhone}?text=${message}`;
    
    output.innerHTML += `${index + 1}. ${contact.name} (${contact.phone}):\n`;
    output.innerHTML += `<a href="${whatsappLink}" target="_blank" style="color: #00ff00; text-decoration: underline; font-weight: bold;">💬 SEND MESSAGE</a>\n`;
    output.innerHTML += `   📱 Formatted: +${whatsappPhone}\n\n`;
});

    
    output.innerHTML += `⚠️  WHATSAPP INSTRUCTIONS:\n`;
    output.innerHTML += `1. Click each "SEND MESSAGE" link\n`;
    output.innerHTML += `2. WhatsApp Web will open in new tab\n`;
    output.innerHTML += `3. Edit message as needed\n`;
    output.innerHTML += `4. Click send button in WhatsApp\n`;
    output.innerHTML += `5. Return here and click next link\n`;
    output.innerHTML += `6. Wait 30 seconds between messages\n\n`;
    
    // Bulk WhatsApp Web link
    output.innerHTML += `🚀 QUICK ACCESS:\n`;
    output.innerHTML += `<a href="https://web.whatsapp.com" target="_blank" style="color: #00ff00; text-decoration: underline; font-weight: bold;">📱 OPEN WHATSAPP WEB</a>\n\n`;
    
    // Log campaign to admin
    const campaignLog = `
📱 WHATSAPP BULK CAMPAIGN INITIATED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👤 User: ${userSession.userName} (${userSession.userEmail})
📊 Stats: ${phoneContacts.length} valid phone numbers
⏰ Time: ${new Date().toLocaleString()}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
